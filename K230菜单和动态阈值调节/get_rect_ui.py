...
def process_blobs(img, blobs, color, pto):
...
def process_and_pack_corners(uart, corners, zuobiao):
...
def run_get_rect(sensor, key_main, key_aux, key_esc, uart,
                 WIDTH=640, HEIGHT=480, thresholds=None):

    pto = YbProtocol()
    mode = 1
    data = 0

    # 默认阈值（与 get_rect.py 相同）
    if thresholds is None:
        thresholds = [
            (67, 100, 4, 40, -12, 24),  # 红色阈值
    ...
    first_rect_corners = [[0, 0]] * 4
    second_rect_corners = [[0, 0]] * 4
    ...
    while True:
        ...
        # 按下主按键切换模式
        if key_main.is_pressed():
            time.sleep(0.1)
            while key_main.is_pressed():
                time.sleep(0.1)
            mode = mode + 1
        if mode >= 6:
            mode = 1  # 模式循环
        ...
        if mode == 1:
            ...
            if blobs:
                process_blobs(img, blobs, detect_color, pto)  # 画框和十字
                # 检测辅助按键，发送色块坐标
                if key_aux.is_pressed():
                    time.sleep(0.1)
                    while key_aux.is_pressed():
                        time.sleep(0.1)
                        send_xy(uart, blobs[0][0], blobs[0][1])  # 发送第一个色块的坐标（高低字节）
                        num = 1
                        #print(num)
            # 串口接收数据
            data = uart.read()
            ...
            if blobs:
                process_blobs(img, blobs, detect_color, pto)  # 画框和十字
                send_xy(uart, blobs[0][0], blobs[0][1])
            #print(data)
            draw_message(img, mode)  # 显示当前模式
            img.draw_rectangle(215, 85, 217, 290, color = (255, 255, 255), thickness=1)
            Display.show_image(img)  # 显示图像
            ...
            if flag_find_second_rect == True:
                ...
                if show_second_rect:
                    # 绘制外框矩形的四条边
                    img.draw_line(second_rect_corners[0][0], second_rect_corners[0][1], second_rect_corners[1][0], second_rect_corners[1][1], color=(255, 255, 255))#by kongcheng
            ...
            if key_aux.is_pressed():
                ...
                while key_aux.is_pressed():
                    time.sleep(0.1)
                process_and_pack_corners(uart, second_rect_corners, first_rect_corners)
            draw_message(img, mode)  # 显示当前模式
